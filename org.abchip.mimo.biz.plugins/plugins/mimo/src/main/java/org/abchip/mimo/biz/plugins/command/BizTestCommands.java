...
//		}
	...
	private void createOrder(Context context, String partyId) {
		...
		//
		// TODO qui richiamare il servizio calcTax per aggiungere l'iva all'ordine (OrderAdjustment) che poi sarà trasferita nella fattura
		//
		
		
		for (OrderItemShipGroupAssoc orderItemShipGroupAssoc : orderItemShipGroupAssocReader.find(filter)) {
	...
	private void createInvoiceItem(Context context, Delegator delegator, LocalDispatcher dispatcher, Invoice invoice, String item, int quantity, String itemType) {
		...
		for ( TaxAuthorityRateProduct taxAuthorityRateProduct : taxAuthorityRateProductReader.find(filterTaxAuth, null, null, 1)) {
...
//		agreementTerm.setDescription();
	...
	private void renewalAgreement(Context context, String agreementId) {
		 ...
		 * ed i termini della varie righe sono scaduti.
		 * verrà presa l'ultima riga e copiata in una nuova con nuove date di termine.
		 */
		Timestamp timeStamp = new java.sql.Timestamp(System.currentTimeMillis());
		
		// TODO gestione della where con date validità da rivedere
		// TODO In EntityUtils dalla specifica 123 alla 141 ci sono i metodi che utilizza Biz per le where con le date
		...
		ResourceReader<Agreement> agreementReader = resourceManager.getResourceReader(context, Agreement.class);
//		ResourceReader<AgreementItem> agreementItemReader = resourceManager.getResourceReader(context, AgreementItem.class);
		...
		for (Agreement agreement : agreementReader.find(filter)) {
			System.out.println("Verify agreement " + agreement.getAgreementId());
			
			String termFilter = "agreementId = '" + agreement.getAgreementId() + "'";

			// TODO leggo i termini di contratto della riga per capire se crearne una nuova. Potrebbe diventare un metodo;
			int open = 0;
			int close = 0;
			
			AgreementTerm agreementTermLast = null;
			for (AgreementTerm agreementTerm : agreementTermReader.find(termFilter)) {
				Date date = new Date();
				if(agreementTerm.getThruDate().compareTo(date) < 0) {
					close++;
//					System.out.println(agreementTerm.getAgreementItemSeqId() + " " + agreementTerm.getFromDate() + " " + agreementTerm.getThruDate() + " " + agreementTerm.getDescription());
				} else {
					open++;
				}
			...
			if (open == 0 && close > 0) {
				...
				for ( AgreementProductAppl agreementProduct : agreementProductApplReader.find(productFilter)) {
				...
				for ( AgreementProductAppl agreementProduct : agreementProductApplReader.find(productFilter)) {
					createInvoiceItem(context, delegator, dispatcher, invoiceEntity, agreementProduct.getProductId().getProductId(), 1, agreementTermLast.getInvoiceItemTypeId().getID());
				}
				...
				if(creditCard != null) {
					...
					PaymentMethod paymentMethod = StripePaymentManager.createPaymentCardMethod(creditCard.getCardNumber(),Integer.parseInt(values[0]), Integer.parseInt(values[1]), creditCard.getDescription());
...
/*
			// lettura righe AgreementItem
			String itemFilter = "agreementId = '" + agreement.getAgreementId() + "'";
			
			// lettura righe AgreementItem
			for (AgreementItem agreementItem : agreementItemReader.find(itemFilter)) {
			}
 */
		}
	}
	...
	private String createPaymentFromInvoice(Context context, Delegator delegator, Invoice invoiceEntity) {
		...
		try {
			...
			org.abchip.mimo.biz.model.accounting.payment.PaymentMethod paymentMethod = PaymentServices.getPaymentMethodParty(context, invoiceEntity.getPartyId().getID(), "CREDIT_CARD");
