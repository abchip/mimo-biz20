 ...
 * Refs: https://docs.microsoft.com/en-us/linkedin/shared/authentication/authorization-code-flow 
 */
...
//    public static final String UserApiUri = "/v1/people/~";
    public static final String UserApiUri = "/v2/me";
    ...
    public static final String ApiTokenEndpoint = "https://api.linkedin.com/v2/me";

    public static final String EmailApiTokenEndpoint = "https://api.linkedin.com/v2/emailAddress?q=members&projection=(elements*(handle~))";
...
//        HttpGet getMethod = new HttpGet(TokenEndpoint + UserApiUri + "?oauth2_access_token=" + accessToken);
        HttpGet getMethod = new HttpGet(ApiTokenEndpoint);
        getMethod.setHeader("Host", "api.linkedin.com");
        getMethod.setHeader("Connection", "Keep-Alive");
        getMethod.setHeader("Authorization", "Bearer " + accessToken);
        Map<String, Object> userInfo = null;
        try {
            userInfo = LinkedInAuthenticator.getUserInfo(getMethod, UtilHttp.getLocale(request));
            if (!UtilValidate.isEmpty(userInfo)) {
            	// retrieve email
                getMethod = new HttpGet(EmailApiTokenEndpoint);
                getMethod.setHeader("Host", "api.linkedin.com");
                getMethod.setHeader("Connection", "Keep-Alive");
                getMethod.setHeader("Authorization", "Bearer " + accessToken);
                String email = LinkedInAuthenticator.getUserEmailInfo(getMethod, UtilHttp.getLocale(request));
    ...
    private static String checkLoginLinkedInUser(HttpServletRequest request, Map<String, Object> userInfo, String accessToken) {
