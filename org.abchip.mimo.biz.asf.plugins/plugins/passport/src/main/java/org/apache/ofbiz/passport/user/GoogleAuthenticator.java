...
import org.apache.ofbiz.passport.event.GoogleEvents;
...
/**
 * Google OFBiz Authenticator
 */
public class GoogleAuthenticator implements Authenticator {

    private static final String module = GoogleAuthenticator.class.getName();

    public static final String props = "googleAuth.properties";
     ...
     * 
     * For Google users, we only check if the username(userLoginId) exists an 
    ...
    public boolean authenticate(String userLoginId, String password, boolean isServiceAuth) throws AuthenticatorException {
        ...
        try {
            ...
            GenericValue googleUser = EntityQuery.use(delegator).from("GoogleUser").where("googleUserId", externalAuthId).queryOne();
            if (googleUser != null) {
                String accessToken = googleUser.getString("accessToken");
                if (UtilValidate.isNotEmpty(accessToken)) {
                    getMethod = new HttpGet(GoogleEvents.AuthorizeUri  + "?oauth2_access_token=" + accessToken);
                    user = GoogleAuthenticator.getUserInfo(getMethod, Locale.getDefault());
                }
            }
        ...
        Debug.logInfo("Google auth called; returned user info: " + user, module);
        return user != null;
    }
    ...
    public void syncUser(String userLoginId) throws AuthenticatorException {
    	Map<String, Object> userMap = getGoogleUserinfo(userLoginId);
    ...
    private Map<String, Object> getGoogleUserinfo(String userLoginId) throws AuthenticatorException {
    	Map<String, Object> user = null;
        HttpGet getMethod = null;
        try {
            GenericValue userLogin = EntityQuery.use(delegator).from("UserLogin").where("userLoginId", userLoginId).queryOne();
            ...
            if (googleUser != null) {
                String accessToken = googleUser.getString("accessToken");
                if (UtilValidate.isNotEmpty(accessToken)) {
                    getMethod = new HttpGet(GoogleEvents.TokenServiceUri + "?oauth2_access_token=" + accessToken);
    ...
    private String createUser(Map<String, Object> userMap, GenericValue system) throws AuthenticatorException {
        ...
        if (userMap.containsKey("given_name")) {
            createPersonUlMap.put("firstName", (String) userMap.get("given_name"));
        }
        if (userMap.containsKey("family_name")) {
            createPersonUlMap.put("lastName", (String) userMap.get("family_name"));
        }
    ...
    public void updatePassword(String username, String password, String newPassword) throws AuthenticatorException {
        Debug.logInfo("Calling Google:updatePassword() - ignored!!!", module);
    }
    ...
    public static String getGoogleUserId(Map<String, Object> userInfo) {
