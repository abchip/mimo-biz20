...
public class GitHubAuthenticator implements Authenticator {
    ...
    public static String getUserEmailInfo(HttpGet httpGet, String accessToken, String tokenType, Locale locale) throws AuthenticatorException {
		...
		try {
	        ...
	        if (getResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
	            ...
	            for (int i = 0; i < jsonArray.length(); i++) {
	                ...
	                String verified = element.getString("verified");
	                String primary = element.getString("primary");
	                String email = element.getString("email");
	                if(verified.equals("true") && primary.equals("true")) {
	                	userEmail = email;
	                }
	            }	            
	        } else {
