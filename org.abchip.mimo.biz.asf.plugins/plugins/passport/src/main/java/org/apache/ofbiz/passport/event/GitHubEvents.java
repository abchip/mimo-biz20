...
public class GitHubEvents {
    ...
    public static final String UserEmailApiUri = "/user/emails";
    ...
    public static String parseGitHubResponse(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            if (!UtilValidate.isEmpty(userInfo)) {
            	// retrieve email
                getMethod = new HttpGet(ApiEndpoint + UserEmailApiUri);
                String userEmail = GitHubAuthenticator.getUserEmailInfo(getMethod, accessToken, tokenType, UtilHttp.getLocale(request));
                if (!UtilValidate.isEmpty(userEmail)) {
...
//        String gitHubUserId = (String) userInfo.get("login");
        String gitHubUserId = (String) userInfo.get("email");
